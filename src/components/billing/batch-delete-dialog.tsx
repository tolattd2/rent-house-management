...
export function BatchDeleteDialog({ months, branches, onClose, onDeleted }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/billing/batch-delete?${params}`)
      .then((r) => r.json())
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-destructive">
            <Trash2 className="w-4 h-4" /> Batch Delete Billings
          </DialogTitle>
        </DialogHeader>
          ...
          <div className={`rounded-lg px-4 py-3 text-sm flex items-start gap-2.5 ${
            ...
            <span>
              {loading ? 'Counting…' : count === null ? '—' : count === 0
                ...
                : `This will permanently delete ${count} billing record${count !== 1 ? 's' : ''} for ${month}${branch !== 'all' ? ` (${branch})` : ''}.`}
            </span>
          </div>
        </div>
